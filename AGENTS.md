...
7. The guest-catalogue brief delivered on the Desktop as `MehmonGo-Codex-Catalog-Final\Codex-TASK.md`. Its `internal/PRICING-RU.md` is confidential: purchase prices, hotel payouts and the bonus formula must never reach the site, the bundle or GitHub.
...
5. Codex cannot hold an admin session (password entry is off limits and injecting a session was blocked), so any further UI checks in the admin are the owner's; the room toggle in the live room editor is the only admin control not yet exercised by hand.
